...
switch ($action) {
    case 'cleanup_brands':
        // Geçici: Nesil sayısı 0 olan ve bölgesel varyant markaları temizle
        $mode = $_GET['mode'] ?? 'preview'; // preview veya execute
        $result = ['mode' => $mode, 'removed' => [], 'kept' => [], 'errors' => []];

        // 1. Tüm markaları nesil sayılarıyla al
        ...
        // Bölgesel varyant kalıpları
        $regional_patterns = [
            '/(FAW)/', '/(GAC)/', '/(SVW)/', '/(SGM)/', '/(SGMW)/', '/(DYK)/',
            '/(CHANGAN)/', '/(DONGFENG)/', '/(BBDC)/', '/(BRILLIANCE)/',
            '/(NANJING)/', '/(SAIC)/', '/(HAINAN)/', '/(ZHENGZHOU)/',
            '/(JIANGXI)/', '/(HUATAI)/', '/(BJC)/', '/(CHERY)/', '/(DF-PSA)/',
            '/(FJDA)/', '/(GAC FCA)/', '/(SOUEAST)/',
            'FORD USA', 'FORD AUSTRALIA', 'FORD ASIA', 'FORD OTOSAN',
            'HOLDEN', 'VAUXHALL', 'PONTIAC', 'OLDSMOBILE', 'PLYMOUTH',
            'MERCURY', 'SATURN', 'GEO', 'EAGLE', 'ASÜNA',
            'HSV', 'FPV', 'SCION',
        ];
        ...
        foreach ($brands as $b) {
            ...
            if (!$shouldRemove) {
                foreach ($regional_patterns as $pat) {
                    if (str_starts_with($pat, '/')) {
                        // Parantezli kalıp
                        if (str_contains($b['name'], trim($pat, '/'))) {
                            $shouldRemove = true;
                            $reason = 'bolgesel_varyant: ' . $pat;
                            break;
                        }
                    } else {
                        if ($b['name'] === $pat) {
            ...
            if ($shouldRemove) {
                $result['removed'][] = [
                    'id' => (int)$b['id'],
                    'name' => $b['name'],
                    'models' => (int)$b['model_count'],
                    'gens' => (int)$b['gen_count'],
                    'reason' => $reason,
                ];
                ...
                if ($mode === 'execute') {
                    try {
                        $mid = (int)$b['id'];
                        // Vehicles sil (model üzerinden)
                        $pdo->exec("DELETE v FROM catalog_vehicles v
                                    JOIN catalog_models mo ON v.model_id = mo.id
                                    WHERE mo.manufacturer_id = $mid");
                        // Models sil
                        $pdo->exec("DELETE FROM catalog_models WHERE manufacturer_id = $mid");
                        // Manufacturer sil
                        $pdo->exec("DELETE FROM catalog_manufacturers WHERE id = $mid");
                    } catch (Exception $e) {
                        $result['errors'][] = $b['name'] . ': ' . $e->getMessage();
                    }
                }
            } else {
                $result['kept'][] = [
        ...
        $result['summary'] = [
            ...
            'to_remove' => count($result['removed']),
            'to_keep' => count($result['kept']),
        ];
