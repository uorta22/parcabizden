...
import type { VehicleInfo, VINDecodeAPIResponse } from '@/types/vehicle'
...
export default function ChassisSearch() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/vin-decode?vin=${chassisNumber.trim()}`)
      const data: VINDecodeAPIResponse = await response.json()

      if (!data.success || !data.data) {
        setError(data.error || 'Arac bilgisi bulunamadi.')
        return
      }
